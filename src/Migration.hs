--singleton, find_top and maximality 
...
is_maximal :: Expr -> Env -> Bool
is_maximal e env = not (null (get_the_next_well_typed_term e env))
...
find_top :: Expr -> Env -> [Expr]
find_top e env = 
  case (check_termination e env) of 
...
findAllMaximalMigrationsUnlimited t env =
  concat 
  . map (unique . filter (\t' -> is_maximal t' env))
  . takeWhile (not . null)
...
findMaximalMigrationsAtDepth n t env =
   [ t' | t' <- findWellTypedMigrationsAtDepth n t env, is_maximal t' env]
...
is_any_term_maximal (x:xs) env = case (is_maximal x env) of
    True -> Just x
